...
class ASPP3D(nn.Module):
    def __init__(self, in_plane, out_plane, reduction=4):
        ...
        self.aspp1 = _ASPPModule3D(in_plane, mid_plane, 1, padding=0, dilation=dilations[0])
        self.aspp2 = _ASPPModule3D(in_plane, mid_plane, (1, 3, 3), padding=(0, dilations[1], dilations[1]), dilation=(1, dilations[1], dilations[1]))
        self.aspp3 = _ASPPModule3D(in_plane, mid_plane, (1, 3, 3), padding=(0, dilations[2], dilations[2]), dilation=(1, dilations[2], dilations[2]))
        self.aspp4 = _ASPPModule3D(in_plane, mid_plane, (1, 3, 3), padding=(0, dilations[3], dilations[3]), dilation=(1, dilations[3], dilations[3]))
        self.conv1 = nn.Conv3d(mid_plane*4, out_plane, kernel_size=(1, 3, 3), padding=(0, 1, 1), bias=False)
...
class SEBasicBlock(nn.Module):
    ...
    def __init__(self, inplanes, planes, stride=1, downsample=None, reduction=8):
        ...
        self.conv1 = nn.Conv3d(inplanes, planes, kernel_size=(1,3,3), padding=(0,1,1))
        self.in1 = nn.InstanceNorm3d(planes)
        self.relu = nn.ReLU(inplace=True)
        self.conv2 = nn.Conv3d(planes, planes, kernel_size=(1,3,3), padding=(0,1,1))
...
class SAM(nn.Module):
    ...
    def seRepeat(self, repeat=2):
        return nn.Sequential(
            *nn.ModuleList([SEBasicBlock(self.indim, self.indim) for _ in range(repeat)])
        )
...
class MemCrompress(nn.Module):
    def __init__(self, repeat=0, norm=True):
        ...
        self.compress_key = nn.Conv3d(64, 64, kernel_size=(2, 3, 3), padding=(0, 1, 1))
        self.compress_value = nn.Conv3d(512, 512, kernel_size=(2, 3, 3), padding=(0, 1, 1))
    ...
    def forward(self, key, value):
        ...
        value = value.flatten(start_dim=0, end_dim=1) # N*O, C, T, H, W [8, 512, 2, 24, 24]
