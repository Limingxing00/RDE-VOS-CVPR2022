...
# from dataset.OSS import Image.open, os.listdir, exists
...
class YouTubeVOSTestDataset(Dataset):
    def __init__(self, data_root, split, res=480):
        ...
        self.videos = []
        self.shape = {}
        self.frames = {} # length 507 in validation set
        ...
        for vid in vid_list:
            ...
            # filter the other file and save the "png" image
            first_mask = [x for x in first_mask if x.endswith("png")][0]
        ...
        if res != -1:
            self.im_transform = transforms.Compose([
                transforms.ToTensor(),
                im_normalization,
                transforms.Resize(res, interpolation=Image.BICUBIC),
            ])

            self.mask_transform = transforms.Compose([
                transforms.Resize(res, interpolation=Image.NEAREST),
            ])
        else:
            self.im_transform = transforms.Compose([
                transforms.ToTensor(),
