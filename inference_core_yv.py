...
class InferenceCore:
    def __init__(self, args, prop_net:RDE_VOS, images, num_objects, top_k=20, 
                    mem_every=5, mode="stm", req_frames=None):
    ...
    def do_pass(self, key_k, key_v, idx, end_idx):
        ...
        for i, oi in enumerate(self.enabled_obj):
            if oi not in self.mem_banks:
                self.mem_banks[oi] = MemoryBank(self.prop_net.mem_compress, k=1, top_k=self.top_k, mode=self.mode)
